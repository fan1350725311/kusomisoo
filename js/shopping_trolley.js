 ...
 */
 window.addEventListener("DOMContentLoaded",function(){
    var section = document.querySelector("#big>section");
    let n = 111;
    //htmlStr用来存储要渲染的html结构
    let htmlStr = "";
    let data = JSON.parse(window.sessionStorage.getItem("shopping_trolley_goods_data"));
    for(let i = 0;i<data.length;i++){
        htmlStr+=`
            <article>
                <aside></aside>
                <figure><img src="${data[i].src}"></figure>
                <div>
                    <h4>${data[i].name}</h4>
                    <span>原价</span>
                    <mark>${data[i].price}</mark>
                </div>
                <footer>
